...
@app.route('/edit_main_task', methods=['POST'])
def edit_main_task():
    if 'user_id' not in session:
        return redirect(url_for('login'))
...
@app.route('/delete_main_task/<int:main_task_id>', methods=['POST'])
...
@app.route('/delete_sub_task/<int:sub_task_id>', methods=['POST'])
...
@app.route('/toggle_sub_task_done/<int:sub_task_id>', methods=['POST'])
...
@app.route('/dashboard_tasks', methods=['GET'])
def dashboard_tasks():
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'Authentication required'}), 401
...
@app.route('/toggle_dashboard_task/<int:task_id>', methods=['POST'])
